...
int main()
{
	...
	if ((fd0_A = open("/dev/emoji_device", O_RDWR)) < 0) perror("open");
	if ((fd0_B = open("/dev/emoji_device", O_RDWR)) < 0) perror("open");
	...
	printf(":bow: > /dev/emoji_device ->%s\n", buff);
	if (read(fd0_B, buff, 16) < 0) perror("read");
	printf("0_B > /dev/emoji_device -> %s\n", buff);
